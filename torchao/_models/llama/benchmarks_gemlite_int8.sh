...
#for batch_size in 4; do
	...
	#python generate.py --checkpoint_path $CHECKPOINT_PATH/$MODEL_REPO/model.pth --quantization int8dq  --write_result $output_file --compile --batch_size $batch_size;
	
	#python generate.py --checkpoint_path $CHECKPOINT_PATH/$MODEL_REPO/model.pth --quantization float8dq  --write_result $output_file --compile --batch_size $batch_size;
